...
export function GroupMembersCard({
    ...
    return (
        <section className={"flex flex-col gap-4 " + (className ?? "")}>
            ...
            <div className="surface-card flex flex-1 flex-col gap-3 p-5 sm:p-6">
                ...
                {totalCount > 3 ? (
                    <footer className="mt-auto flex items-center pt-1">
                        <Button variant="ghost" size="sm" trailingIcon={<ArrowRightIcon />} asChild>
                            <Link to={`/groups/${groupId}/members`}>View all {totalCount}</Link>
                        </Button>
                    </footer>
                ) : null}
            </div>
